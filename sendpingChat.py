...
def main():
    ...
    destIP="192.168.122.192"
    with open('messagef.txt', 'r') as tranfer:
		for text in tranfer:
			for x in text.split():
				sendTime = sendPing(destIP, x)
				if sentTime == 0:
    ...
    while exitLoop == False:
        ...
        if selection == 'S' or selection == 's':
            ...
            destinationIp="192.168.122.192"
			with open('messagef.txt', 'r') as tranfer:
				for text in tranfer:
						for x in text.split():
							sendTime = sendPing(destinationIP, x)
							if sentTime == 0:
								print "Failed to send the message. Verify the IP is correct."
							else:
								print "Message Sent Successfully @ " + str(sentTime)
